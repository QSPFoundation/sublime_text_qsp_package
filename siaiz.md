...
1. Обязательно скачайте [классический плеер QSP последней версии](https://github.com/QSPFoundation/qspgui/releases) и разместите его в папке "`C:\Program Files\QSP\qsp590`". Делается это так:
	* Скачайте [zip-архив с плеером](https://github.com/QSPFoundation/qspgui/releases/download/v5.9.5-b1/qspgui-5.9.5-b1-win32.zip);
	* Откройте папку "`C:\Program Files`";
	* В ней создайте папку "`QSP`" и откройте;
	* В ней создайте и откройте папку "`qsp590`";
	* Откройте ранее скачанный zip-архив, найдите в нём папку '`bin`' и все файлы из неё скопируйте (можно просто перетащить) в папку "`qsp590`".
