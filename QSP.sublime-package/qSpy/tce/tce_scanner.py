...
from .tokens import TextToken as tkn, TokenNode
from .tokens import TextTokenType as tt
...
class TceScanner:
    ...
    def __init__(self, qsps_lines: List[QspsLine]) -> None:
        ...
        self._cur_line:QspsLine = self._qsps_lines[0] # значение текущей строки
        ...
        self._open_location:bool = False
    ...
    def scan_tokens(self) -> None:
        ...
        if self._curlexeme and self._scan_funcs:
            raise er.TceScannerRunError(f"no clean handler stack {''.join(self._curlexeme)}, {[foo.__name__ for foo in self._scan_funcs]}")
    ...
    def _qsps_file_expect(self, c:str) -> None:
        ...
        elif self._curchar_is_last_in_line():
            self._add_token(tt.RAW_LINE)
        else:
            self._scan_funcs.append(self._raw_line_end_expect)
    ...
    def _loc_body_scan(self, c:str) -> None:
        ...
        if cn == 0 and c == '-' and self._next_in_line() == '-':
            ...
            self._scan_funcs.append(self._apostrophe_string_literal_expect)
              ...
              not self._curchar_is_last_in_line()):
    ...
    def _quote_string_literal_expect(self, c:Char) -> None:
        ...
        if c == '"':
            if self._next() == c:
                ...
                return
            self._add_token(tt.TEXT_QUOTE_CONST)
            self._scan_funcs.pop()
    ...
    def _apostrophe_string_literal_expect(self, c:Char) -> None:
        ...
        if c == "'":
            if self._next() == c:
                ...
                self._scan_funcs.append(self._escape_expect)
                return
            self._add_token(tt.TEXT_APOSTROPHE_CONST)
            self._scan_funcs.pop()
    ...
    def _curchar_is_last_in_file(self) -> bool:
    ...
    def _next(self, char_num:int=-1) -> Char:
        ...
        if char_num + 1 < self._line_len:
            return self._cur_line[char_num + 1]
        elif self._line_num + 1 < len(self._qsps_lines):
            return self._qsps_lines[self._line_num + 1][0]
        return '\0'

    # обработчик ошибок. Пока просто выводим в консоль.
    def _error(self, message:str) -> None:
        print(f"Dirs-Scanner. {message}: ({self._line_num}, {self._curchar}).")
