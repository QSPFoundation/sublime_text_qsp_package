...
<div style="text-align:right;">QSP-Package for Sublime Text</div>
...
- Изменён формат файлов "`project.json`" ("`qsp-project.json`"). ~~Подробнее см. Изменения в json-файле.~~

## v.0.16
**Исправления:**
- Некоторые фиксы работы с QGC и ключей в проектных файлах
- исправлена подсветка синтаксиса для многострочных условий
- актуализирована подсветка комментариев для многострочных операторов
**Изменения:**
- Обновлено до версии libqsp 5.9.0:
	- добавлена поддержка новых операторов и функций `sortarr`, `scanstr`, `curobjs`;
	- добавлена поддержка кортежей.
- Добавлен параметр `assets` для `qsp-project.json`.
- Добавлена подсветка QSPdocs - условного описания параметров локации-функции. В планах подтягивать параметры при наведении на название функции в других файлах проекта и выводить попап-подсказки.
- Выделена отдельная папка с примерами подсветки синтаксиса.

## v.0.17

Масштабный апдейт подсветки синтаксиса и некоторых скриптов в связи с критическими обновлениями Sublime Text.

Вообще, это чисто техническое обновление, цель которого — сделать код более читабельным и поддерживаемым, чтобы в дальнейшем было проще вносить правки. Функционал плагина не должен был измениться, разве что я добавил поддержку новых ключевых слов в подсветке и сниппетах, да исправил грубые ошибки, приводившие к неверному учёту и подсчёту локаций.

Всё ещё это требует множества тестов, поэтому версия никак не сдвинется с 0.xx.

**Исправления:**
- Множество правок подсветки синтаксиса. До глобальных изменений дело снова не дошло, но я к этому стремлюсь. Возможно скоро.
- Исправлена логика поиска дубликатов локаций, а так же работы с WorkSpace проекта на QSP.
- Масштабный рефакторинг кода с упрощением многих частей скриптов и номинальным ускорением работы.
**Изменения:**
- Обновлено до версии libqsp 5.9.2.
