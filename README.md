...
Элемент "save_temp_files" разрешает оставлять промежуточные файлы после сборки (т.е. рядом с каждым собранным файлом "`.qsp`" будет лежать его исходник в формате qsps с раширением "`.txt`"). Ему можно назначать значения `true` и `false`. Значением по умолчанию является `false`.
...
Элемент "start_qsploc_file" позволяет указать локацию, с которой должна начинать игра. Значение подчиняется тем же правилам, что и `files`.

```json
"start_qsploc_file": "start.qsps"
```
...
1. Файл, указанный в `start_qsploc_file`.
