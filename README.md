...
6. Директивы записанные в многострочных комментариях являются частью комментария.
7. Самостоятельная невалидная директива воспринимается препроцессором, как спец-комментарий.
...
7. Объявляя метку через не объявленную метку, вы тем самым создаёте обе метки: `var(one = two)`
8. `True` и `False` — это метки, которые определены заранее по умолчанию. Их нельзя переопределить. В отличие от всех прочих меток, они всегда ссылаются на булевые значения, поэтому с их помощью можно явно проверять существование меток: `if(build_em!=True)` или `if(build_em==False)`. Объявленная метка при сравнении с `True` вернёт `True` оператору условия. Однако рекомендую проверять существование метки, написав просто: `if(build_em)`.
9. При обработке препроцессором все строки между локациями убираются из конечного файла. Это немного ускоряет конвертирование при сборке.
10. Если вы используете многострочные текстовые значения в команде QSP, а в конце записан спец-комментарий с удалением, удалится не только строка с таким спец-комментарием, но и вся команда.
11. Даже в режиме "Off" препроцессор продолжает обрабатывать файлы, поэтому могут пропадать пустые строки в коде локаций.
