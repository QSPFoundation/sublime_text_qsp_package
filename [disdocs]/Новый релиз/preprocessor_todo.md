...
4. Все объявленные метки будут глобальны для всего проекта. Будьте внимательны при организации работы препроцессора.
5. ВЛОЖЕННОСТЬ БЛОКОВ УСЛОВИЙ НЕ ПОДДЕРЖИВАЕТСЯ!
6. `True` и `False` — это метки, которые определены заранее по умолчанию. Их нельзя переопределить. Используются, как значения для проверки существования метки: `if(build_em!=True)` или `if(build_em==False)`.
7. При обработке препроцессором все строки между локациями убираются из конечного файла. Это немного ускоряет конвертирование при сборке.
8. Если вы используете многострочные текстовые значения в команде QSP, а в конце записан спец-комментарий с удалением, удалится не только строка с таким спецкомментарием, но и вся команда.
